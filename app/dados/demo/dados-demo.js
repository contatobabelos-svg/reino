...
(function () {
  ...
  const PESSOAS = [
    ...
    { nome: "Marcelo", titulo: "Imperador", cidade: "Campinas" },
  ...
  if (DB) {
    ...
    // [UI kit] Complemento por município (js/demo-municipios.js): todo município do IBGE
    // ganha 1–4 empresas fictícias, para nenhum estado ou cidade aparecer vazio no globo.
    const DM = window.DEMO_MUNICIPIOS;
    if (DM) DM.forEach(([cidade, uf, lat, lng, es], ci) => es.forEach(([nome, ni, ai, dono, semanas, bi], k) => {
      const n = 1 + Math.floor(rnd() * 5);
      const avaliadores = Array.from({ length: n }, () => base + Math.floor(rnd() * DB.pessoas.length));
      const notas = avaliadores.map(() => (rnd() < 0.5 ? 5 : rnd() < 0.7 ? 4 : rnd() < 0.85 ? 3 : 2));
      const p = PESSOAS[base + dono];
      empresas.push({ id: `demo-mun-${ci + 1}-${k + 1}`, nome, nicho: DB.nichos[ni], cidade, bairro: DB.bairros[bi], uf, estado: UF[uf], regiao: REGIAO_UF[uf], lat, lng, abrangencia: DB.abrangencias[ai], dono: p.nome, titulo: p.titulo, semanas, avaliadores, notas });
    }));
  }

...
- [x] A5 Domínio próprio escolhido: `networkreino.com` — adicionado no Vercel (`vercel domains add networkreino.com`). **Aguarda DNS no Registro.br**
- [ ] A5b DNS: apontar `networkreino.com` para os nameservers da Vercel (ou CNAME) no Registro.br
...
## AL — "ative o reino - preciso analisar a possibilidade de nós trabalharmos com duas paginas, a de login e a do reino mesmo. isso devido estar lento para entrar após o login por conta dos usuarios. ai após logar, se vai para a pagina. como se ao entrar no login a pagina do reino estivesse off dormindo e ao logar, ela recebe um sinal para acordar." → "pré aquecimento." (22/09)
- [x] AL1 `app/entrar.html` novo: só o login (LoginImersivo), sem o dashboard nem as 25 telas — head bem mais leve que o `index.html` (sem `_ds_bundle` de mapa/bolsa/etc. extra, sem `dados/municipios-tudo.js` de 2,7 MB, sem os serviços/telas do dashboard)
- [x] AL2 Pré-aquecimento: `entrar.html` cria (em tempo ocioso) um iframe oculto com `index.html?aquecendo=1` — o Reino carrega e transpila tudo (inclusive o `_ds_bundle.js` de 4,8 MB) dormindo, em segundo plano, enquanto a pessoa ainda está digitando o login
- [x] AL3 Sinal de acordar: `App.jsx` avisa `reino:pronto` por `postMessage` assim que termina de montar dentro do iframe; ao logar, `entrar.html` manda `reino:acordar` com a sessão pronta (`servicos/contas.js` ganhou `assumir(sessao)` para o iframe herdar a sessão sem depender de reler o localStorage) e revela o iframe — sem precisar recarregar nada
- [x] AL4 Sem tempo de aquecer (rede lenta) ou sem iframe: cai para `location.href = "index.html"`, a navegação de sempre — o login nunca fica travado esperando o mecanismo
- [x] AL5 `index.html` continua funcionando sozinho, do jeito de sempre (link de e-mail, sessão salva, acesso direto) — o modo dormindo só liga com `?aquecendo=1` **e** dentro de um iframe (visita direta nesse link cai no login normal, não trava em tela branca)
...
- [x] AN7 Publicar em produção (migração + funções + site) e teste ponta a ponta: cadastra com 5 campos, entra como demonstração, adm aprova, vira membro com acesso ao Reino sem as telas de adm — publicado 23/09: migrações aplicadas, funções republishadas, site no ar com "ETAPA 1 DE 5·NOME"; teste em produção via API: cadastro novo cria conta `aguardando` (a7c4ac4d, logo apagada), reentrada pelo mesmo WhatsApp reconhece a mesma conta e atualiza nome/empresa/nicho/cidade, linha em `cadastros` gravada (cadeia/pai); Playwright em networkreino.com: entrar.html renderiza, carregamento completo sem 404

## AN+ — "tornar usuario e senha obrigatorios no cadastro de novo, mas no cadastro completo depois de entrar" (23/09)
- [x] AN+1 `reino-cadastro`: usuário e senha obrigatórios — valida no servidor (usuário 3–20 minúsculas/letras+números/_ e não reservado; senha 8–72), reentrada grava usuário+senha na conta existente, conta nova nasce com a senha digitada; erro `senha_fraca` mapeado para o campo `senha` (GoTrue de produção rejeita senha pwned com `weak_password`; o local não). **Publicada em produção 23/09**
- [x] AN+2 `LoginImersivo.jsx`: carrossel agora com 8 passos/7 rótulos — nome → whatsapp → empresa → nicho → cidade → usuário → senha → senha2 (confirmação); senha nunca volta preenchida ao voltar o passo; ROTULOS "Etapa X de 7"
- [x] AN+3 `e2e-cadastro-simples-local.cjs` e `e2e-cadastro-cidade-uf.cjs`: reescritos para o fluxo de 7 campos — verde local 23/09
- [x] AN+4 Publicado + validado em produção: commit `1c3e3b6` no `main` → Vercel; Playwright em o-reino.vercel.app — TUDO PASSOU: abre no cadastro, etapas 2/5/6/7 de 7, cadastro completo entra no app (dashboard no iframe `?aquecendo=1`), função criou a conta `ok:true`, usuário logado é a conta certa, relogar por usuário+senha entra de novo, reino-login respondeu ok, sem erros de console
- [x] AN+5 Limpeza de produção: apagadas 168 contas de teste (154 `teste-*@contas-teste.reino.invalid`, 2 de diagnóstico SMTP/AD5b, 10 `validacao*`/`Valida Publica Teste`, `teste-login@reino.test` e remanescentes `Teste Smoke*`) e seus cadastros/guilda/sessões/tokens; sessões e refresh_tokens zerados (contas reais relogam)

## AO — auditoria profunda de segurança + "resolva o que você consegue sem precisar de mim e traga um listaço que fez" (24/09)
Auditoria completa (RLS, edge functions, auth, frontend, deploy, git) entregue no chat. O que foi corrigido aqui (local, sem push):
- [x] AO1 **CRÍTICA resolvida** — toma de conta pela reentrada por WhatsApp: `reino-cadastro` NUNCA mais sobrescreve a senha de conta que já tem a dela. Conta madura (`perfis.senha_definida`) só abre a sessão com a senha ATUAL (`signInWithPassword`, `senha_nao_confere` no campo senha); o perfil vira fill-only. Conta legada (nasceu com senha aleatória no período AJ/AN) tem UMA janela para adotar a senha digitada e passa a madura. Limite de reentradas por conta (10/5min). **Testado local ponta a ponta** (servi a função no stack local): conta nova ok, reentrada com senha errada → `senha_nao_confere`, com a certa → `ok:true reentrou:true`; conta convertida em legada reentra uma vez (marca `senha_definida=true`) e depois senha errada nega; usuário+senha novos entram pelo reino-login.
- [x] AO2 Migração `supabase/2026-09-24_reentrada_exige_senha.sql` — coluna `perfis.senha_definida` (default true), backfill da coorte legada (e-mail interno `m-%@contas.reino.invalid` que ainda não reentrou → false), `proteger_perfil` bloqueia a própria conta de voltar true→false, `criar_perfil` ganha whitelist de títulos (nada fora da lista vira null). **Aplicada e verificada no local** (12 contas legadas; trigger testado: INSERT força true, true→false bloqueado, false→true liberado).
- [x] AO3 Migração `supabase/2026-09-24_codigos_sem_user_id_anon.sql` — anônimo só lê a coluna `codigo` (não vê mais `user_id`/`nome`); `codigos_editar` perde o `with check ... or pai is null` (brecha que deixava qualquer autenticado editar o código raiz/admin). **Verificado no local**: anon `select=codigo` 200, `select=codigo,user_id` → 42501.
- [x] AO4 `app/servicos/contas.js` — `codigoLivre` pede `codigo` sem sessão e `codigo,user_id` logado; `trocarSenha` marca `senha_definida=true` depois de trocar (tramita no gatilho novo).
- [x] AO5 `vercel.json` — headers de segurança: `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, CSP `frame-ancestors 'self'` (conservadora).
- [x] AO6 `app/componentes/ReinoMapa.jsx` — link do site da empresa só renderiza quando `site` começa com `http(s)://` (o resto das URLs do app é dado estático ou já filtrado pelo `reino-apis`).
- [x] AO7 Estrutura do site montada (`sh montar-site.sh`) e checagens Dominic sem bloqueio; `node --check` ok.
- [ ] AO8 Aplicar as duas migrações e republicar as funções em PRODUÇÃO e dar push no `main` (Vercel) — **precisa do fundador** (sem credencial de banco em mãos)
- [ ] AO9 Fechar 100% a janela de reentrada das contas legadas com OTP por SMS/WhatsApp — precisa de provedor (decisão + chave)
...
## AP — landing de divulgação ("vamos fazer isso localmente", 24/09)
Landing pública em `/` (marketing de pré-cadastro; login segue em `/entrar.html`) criada como `app/landing.html`, herdando o design system (Exo 2/Inter, tokens, hg-*).
- [x] AP1 Seções da proposta do cliente: hero (acrônimo R-E-I-N-O), como funciona (3 passos), recursos (Match, Busca reversa, Busca inteligente, Clube, Score, rede social, Revista, eventos/rodadas, Guildas), Babel com demo de métricas rotulada "exemplo", tabela dos 7 títulos (escolhe/conquista), Primeira Corte (Livro do Reino, 40% melhor, 23/10), Código do Reino (honra), público MEI→indústria, Carta de Apresentação
- [x] AP2 CTA "Segure para abrir o portão" com segurar-para-abrir (progresso), clique curto também navega; lê `?ref=` e leva o afiliado para o cadastro (± `?ref=`)
- [x] AP3 `vercel.json`: "/" agora redireciona para `/landing.html`; `/r/:codigo` → `/?ref=` preservado (chega na landing e repassa ao cadastro); headers AO5 mantidos
- [x] AP4 Site montado (`sh montar-site.sh`) e servido local (http://localhost:8123) para revisão
- [ ] AP5 Decisões do fundador antes do ar: a linha "condição 40% melhor" (número de oferta; CLAUDE.md diz que número novo de preço/percentual é disputa dos sócios) e a Busca reversa seguir apenas como promessa na landing ou virar tela real no app. Publicar = push no `main` (regra: só com pedido explícito)

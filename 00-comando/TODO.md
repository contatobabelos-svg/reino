...
## AQ — botões "Fazer cadastro / Já tenho conta" dentro da tela de login (24/09)
Fundador: quer a opção de "já tenho conta ou cadastrar" na própria tela de login (a paisagem), e não uma página à parte.
...
- [x] AR1 `login-imersivo.css`: botões da escolha baixos (56px, ícone + texto na mesma linha, formato de botão).
- [x] AR2 Apagados por padrão (`opacity .78`, saturação/brilho reduzidos); no hover/foco acendem (brilho cheio, leve elevação) e um anel de luz neon gira na borda (`@property --hg-neon` + `hg-li-neon-roda`).
- [x] AR3 `FundoReino.jsx` (Ceu/SHIPS): 5 naves cruzam o céu de ponta a ponta (entram fora da tela num lado e saem do outro); o fade só acontece pegadinho nas bordas (antes sumiam no meio do caminho). Vídeo re-renderizado com `renderizar-fundo.cjs` (420 quadros; WebM 4,95 MB + MP4 4,07 MB).
- [x] AR4 `LoginImersivo.jsx` + CSS: saída nova em fases — `voo` (a câmera escala para o alto, olhando o céu), `flash` (riscos de luz na velocidade máxima), `escuro` (tela preta), aí a página entra. O clarão antigo (`portal-abre`/`portal-nucleo`) saiu; depois o fundador pediu para tirar o portal também, ficando só zoom + flash.

...
(function () {
  ...
  function criar(raiz, opts) {
    ...
    async function carregarMunicipios(uf) {
      if (!cacheMun[uf]) cacheMun[uf] = fetch(`dados/municipios/${uf}.json`).then((r) => r.json()).then((d) => d.cidades);
      return cacheMun[uf];
    }
    ...
    async function irEnderecoDoUsuario() {
      const u = st.usuario;
      if (!u) return;
      const seq = ++seqVoo;
    ...
    async function renderPainel() {
      let h = "";
      if (st.nivel === "terra") {
        h = cabecalho("Planeta Terra", "O Reino começa no Brasil", "Gire o globo e toque no Brasil para entrar. Na Lua fica a torre da Babel.")
          + `<button class="hg-btn is-cyan is-block" data-ir="brasil">Entrar no Brasil</button>`
          + `<button class="hg-btn is-block hg-btn-lua" data-ir="lua">Visitar a torre Babel na Lua</button>`;
    ...
    function laco(t) {
      ...
      if (sujo || vivo) { desenhar(t); sujo = false; }
    ...
    new IntersectionObserver(([en]) => { const v = en.isIntersecting; if (v && !rodando) { rodando = true; requestAnimationFrame(laco); } else if (!v) rodando = false; }).observe(raiz);
    ...
    const api = { irBrasil, irTerra, irEstado, voltar, definirUsuario, irEnderecoDoUsuario };
    raiz.globo = api;
    return api;
  }

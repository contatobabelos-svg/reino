...
- `design-system/tokens/*.css` e `design-system/components/babel-ui.css` (use os tokens e as classes `hg-*`; não redefina cores)
- `app/estilos/app-shell.css` e `app-layout.css` (regras de rolagem e layout) antes de mexer em qualquer tela
- A tela que vai mudar, em `app/telas/*Screen.jsx`, e o `app/nucleo/App.jsx` (rotas)

## Padrões e proibições
- JSX via Babel no navegador, sem build. Script novo entra numa subpasta de `app/` e no `app/index.html`; nada de `arquivo/` vai ao ar. Teste com `sh montar-site.sh` e sirva `site/` numa porta própria.
- Animar só `transform` e `opacity`; tempos de `design-system/tokens/motion.css`; respeite `prefers-reduced-motion`.
